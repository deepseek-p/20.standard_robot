...
改后：
```c
#define PITCH_SPEED_PID_KI        10.0f   // 恢复 Ki, 陀螺仪无 LPF 延迟不再耦合
```

**注意**：Ki 的具体值需要实测调参。初始恢复 Ki=10（原始值），通过 MCP 遥测在线微调。如果 Ki=10 太大，降到 5 起步。
...
| Ki 恢复后新的振荡模式 | 通过 MCP 在线调参，从 Ki=5 起步逐步增加 |
| 前馈行为变化 | 稳态完全相同（speed=0 → I_pid=0 → 不适应）；动态更好（不被加速度骗） |
| IMU 松动/朝向变化 | 运维问题，换位置但保持朝向不变则无影响 |
...
| `gimbal_task.h` | 1 处：`PITCH_SPEED_PID_KI` 恢复为 10.0f（1 行） |
| **总计** | **2 处，~4 行代码** |
